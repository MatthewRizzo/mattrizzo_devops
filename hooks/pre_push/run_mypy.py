...
def run_mypy(package: str, repo_top_dir: Path, is_verbose: bool) -> int:
    """Runs mypy on the given package and returns if it succeeded or not"""
    try:
        mypy_res = check_output(
            ["mypy", "-p", package], stderr=STDOUT, cwd=repo_top_dir, encoding="utf-8"
        )
        print(mypy_res)
...
def check_mypy() -> int:
    """Uses CLI args to run mypy on the given package"""
    ...
    print(f"Running mypy for {args.package}")
